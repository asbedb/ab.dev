    ...
    return(<>                  
                {postContent.length > 0 ? (
                    postContent.map((post, index) => (
                        <Link key={index} href={post.link} target="_blank" className="flex flex-col flex-wrap w-full">
                            <Card className={`bg-primary-200 text-primary-foreground h-60 w-full hover:cursor-pointer hover:animate-pulse hover:bg-primary-50`}>
                                <CardHeader className="flex flex-col items-start align-middle gap-2 w-full">
                                        ...
                                        <span className="whitespace-normal  text-xl md:text-3xl"> {post.pulledFrom == "GitHub"? "🖥️": "📰"}{post.title}</span>
                                        <h4 className="flex font-bold text-md truncate line-clamp-1 text-secondary-400 ">{post.category || 'None'}</h4>
